...
class my_Continuous_MountainCarEnv(gym.Env):
    """
    ## Description
    ...
    * v0: Initial versions release
    ...
    def __init__(self, render_mode: Optional[str] = None, goal_velocity=0, ego_strength=0.8, adv_strength=0.2):
        self.ego_strength = ego_strength
        self.adv_strength = adv_strength
        self.c_min_action = -ego_strength
        self.c_max_action = ego_strength
        self.d_min_action = -adv_strength
        self.d_max_action = adv_strength
        ...
        self.action_space = spaces.Box(
            low=(self.c_min_action + self.d_min_action), high=(self.c_max_action + self.d_max_action), shape=(1,), dtype=np.float32
        )
    ...
    def step(self, action: np.ndarray):
        ...
        c_act = action[0].flatten()[0]
        d_act = action[1].flatten()[0]
        force = min(max(c_act, self.c_min_action), self.c_max_action)
        d_force = min(max(d_act, self.d_min_action), self.d_max_action)
        velocity += (force + d_force) * self.power - 0.0025 * math.cos(3 * position)
        ...
        if terminated:
            reward = 100.0
        reward = reward - math.pow((c_act+d_act), 2) * 0.1

        reward = reward - math.pow(c_act,2)*.1 + math.pow(d_act,2) * .1
