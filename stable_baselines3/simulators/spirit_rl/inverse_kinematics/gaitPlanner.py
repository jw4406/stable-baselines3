...
class trotGait:
  ...
  def loop(self, V, angle, Wrot, T, offset, bodytoFeet_):
    ...
    step_coord = self.stepTrajectory(
        self.phi + offset[0], V, angle, Wrot, np.squeeze(np.asarray(bodytoFeet_[0, :]))
    )  #FR
    ...
    step_coord = self.stepTrajectory(
        self.phi + offset[1], V, angle, Wrot, np.squeeze(np.asarray(bodytoFeet_[1, :]))
    )  #FL
    ...
    step_coord = self.stepTrajectory(
        self.phi + offset[2], V, angle, Wrot, np.squeeze(np.asarray(bodytoFeet_[2, :]))
    )  #BR
    ...
    step_coord = self.stepTrajectory(
        self.phi + offset[3], V, angle, Wrot, np.squeeze(np.asarray(bodytoFeet_[3, :]))
    )  #BL
