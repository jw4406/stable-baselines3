...
class systemStateEstimator:
  ...
  def controls(self):
    ...
    contactForces = np.transpose(
        np.matrix([
            forceFR[0], forceFR[1], forceFR[2], forceFL[0], forceFL[1], forceFL[2], forceBR[0], forceBR[1], forceBR[2],
    ...
    #draw force vector
    p.addUserDebugLine(
        footPosFR[4], footPosFR[4] - forceFR/100, lineColorRGB=[0, 1, 0], lineWidth=3, replaceItemUniqueId=self.IdFR
    )
    p.addUserDebugLine(
        footPosFL[4], footPosFL[4] - forceFL/100, lineColorRGB=[0, 1, 0], lineWidth=3, replaceItemUniqueId=self.IdFL
    )
    p.addUserDebugLine(
        footPosBR[4], footPosBR[4] - forceBR/100, lineColorRGB=[0, 1, 0], lineWidth=3, replaceItemUniqueId=self.IdBR
    )
    p.addUserDebugLine(
        footPosBL[4], footPosBL[4] - forceBL/100, lineColorRGB=[0, 1, 0], lineWidth=3, replaceItemUniqueId=self.IdBL
    )
  ...
  def footJacobian(self, boxId, foot_index):
    ...
    result = p.getLinkState(boxId, foot_index, computeLinkVelocity=1, computeForwardKinematics=1)
    link_trn, link_rot, com_trn, com_rot, frame_pos, frame_rot, link_vt, link_vr = result
    zero_vec = [0.0] * len(mpos)
    #3x18 (6 body dof, 3dof per leg)
    linearJ, rotJ = p.calculateJacobian(
        boxId, foot_index, localPosition=com_trn, objPositions=mpos, objVelocities=zero_vec, objAccelerations=zero_vec
    )
