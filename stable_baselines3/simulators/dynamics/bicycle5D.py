...
class Bicycle5D(BaseDynamics):
  ...
  def integrate_forward_jax(self, state: DeviceArray, control: DeviceArray) -> Tuple[DeviceArray, DeviceArray]:
    ...
    def crit_delta(args):
      ...
      def crit_delta_vel(args):
        ...
        def vel_then_delta(args):
          ...
          state_tmp2 = self._integrate_forward_dt(state_tmp1, jnp.array([0., ctrl_clip[1]]), c_delta - c_vel)
          return self._integrate_forward_dt(state_tmp2, jnp.zeros(2), self.dt - c_delta)

        def delta_then_vel(args):
          state_tmp1 = self._integrate_forward_dt(state, ctrl_clip, c_delta)
          state_tmp2 = self._integrate_forward_dt(state_tmp1, jnp.array([ctrl_clip[0], 0.]), c_vel - c_delta)
          return self._integrate_forward_dt(state_tmp2, jnp.zeros(2), self.dt - c_vel)
      ...
      def crit_delta_only(args):
        ...
        return self._integrate_forward_dt(state_tmp, jnp.array([ctrl_clip[0], 0.]), self.dt - c_delta)
    ...
    def non_crit_delta(args):
      ...
      def crit_vel_only(args):
        ...
        return self._integrate_forward_dt(state_tmp, jnp.array([0., ctrl_clip[1]]), self.dt - c_vel)
  ...
  @partial(jax.jit, static_argnames='self')
  def cont_deriv(self, state: DeviceArray, control: DeviceArray) -> DeviceArray:
  ...
  @partial(jax.jit, static_argnames='self')
  def _integrate_forward_dt(self, state: DeviceArray, control: DeviceArray, dt: float) -> DeviceArray:
