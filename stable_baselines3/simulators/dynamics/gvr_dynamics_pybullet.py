    ...
    while True:
      ...
      if rotate is None:
        if self.rotate_reset:  # Resets the x, y, z.
          rotate = p.getQuaternionFromEuler(
              np.concatenate(((np.random.rand(2) - 0.5) * np.pi * 0.125, np.array([np.random.uniform(0.0, 2 * np.pi)]
                                                                                 )), axis=0)
          )
        else:
      ...
      self.robot = GVR(self.client, height, rotate, envtype=self.envtype, **kwargs)
      ...
      if has_adversarial:
        ...
        if self.link_name is not None:
          self.adv_debug_line_id = p.addUserDebugLine(
              position_vector, position_vector + force_vector * self.force, lineColorRGB=[0, 0, 1], lineWidth=2.0,
              physicsClientId=self.client, parentObjectUniqueId=self.robot.id,
