...
class Go2DynamicsPybullet(BasePybulletDynamics):
  ...
  def get_random_joint_value(self):
    if self.action_type == "increment":
      return (
          np.random.uniform(self.abduction_min, self.abduction_max), 0.6 + np.random.uniform(-0.5, 0.5),
          -1.5 + np.random.uniform(-0.5, 0.5), np.random.uniform(self.abduction_min, self.abduction_max),
          0.6 + np.random.uniform(-0.5, 0.5), -1.5 + np.random.uniform(-0.5, 0.5),
          np.random.uniform(self.abduction_min, self.abduction_max), 0.6 + np.random.uniform(-0.5, 0.5),
          -1.5 + np.random.uniform(-0.5, 0.5), np.random.uniform(self.abduction_min, self.abduction_max),
          0.6 + np.random.uniform(-0.5, 0.5), -1.5 + np.random.uniform(-0.5, 0.5)
      )
      ...
      if i % 3 == 0:
        ...
        if self.abduction_min <= current_joint_pos[i] + increment <= self.abduction_max:
          ...
          clipped_control.append(
              np.clip(current_joint_pos[i] + increment, self.abduction_min, self.abduction_max)
              - current_joint_pos[i]
          )
      elif i % 3 == 1:
