...
model = A3C_rarl("MlPAACPolicy", env='my_pendulum', verbose=2, normalize_advantage=False,gae_lambda=.9,ent_coef=0.0,max_grad_norm=.5,n_steps=8,vf_coef=.4,gamma=.9,v_learning_rate=1e-4, c_learning_rate=1e-3,d_learning_rate=5e-3, use_sde=True,use_rms_prop=False)
#model = A3C_rarl.load("adv_pendulum_take_6.zip", env=env)
model.learn(total_timesteps=1_500_000)
#model.save("adv_pendulum_take_6.zip")
