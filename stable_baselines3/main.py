...
model = A3C_rarl("MlPAACPolicy", env='my_pendulum', verbose=2, normalize_advantage=False,gae_lambda=.9,ent_coef=0.0,max_grad_norm=.5,n_steps=8,vf_coef=1,gamma=.9,v_learning_rate=1e-5, c_learning_rate=1e-4,d_learning_rate=5e-4, use_sde=True,use_rms_prop=False)
#model = A3C_rarl.load("adv_pendulum_take_10.zip", env=env)
#model.v_learning_rate = 1e-9
...
#callback_on_best = StopTrainingOnRewardThreshold(reward_threshold=-1, verbose=1)
#eval_callback = EvalCallback(env, callback_on_new_best=callback_on_best, verbose=1)
model.save("adv_pendulum_take_0.zip")
