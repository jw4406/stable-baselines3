...
model = SMART("MlPAACPolicy", use_stackelberg=True, dstb_action_space=Box(-.3, .3, (1,), dtype=np.float32), learning_starts=100, env=env, verbose=2, policy_kwargs={'net_arch': dict(pi=[16,16], qf=[64,64])}, v_learning_rate=linear_schedule(5e-4), c_learning_rate=linear_schedule(1e-3), d_learning_rate=linear_schedule(5e-3),buffer_size=25000, batch_size=128, train_freq=32, gradient_steps=32, gamma=0.999, tau=0.01, use_sde=True)
model = SMART.load("./stsac_autograd_smartbias.zip", env=env)
#model=A2C("MlpPolicy", normalize_advantage=True, verbose=2, env=env, n_steps=100, learning_rate=linear_schedule(3e-4), use_sde=True, use_rms_prop=False)
...
checkpoint_callback = CheckpointCallback(
  save_freq=1000,
  save_path="./logs/",
  name_prefix='stsac_test_full_autograd',
)
...
#callback_on_best = StopTrainingOnRewardThreshold(reward_threshold=-200, verbose=1)
#eval_callback = EvalCallback(env, callback_on_new_best=callback_on_best, verbose=1)
model.save("stsac_autograd_smartbias.zip")
